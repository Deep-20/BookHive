...
const AuthorPage = () => {
  ...
  const [authors, setAuthors] = useState([]); // Stores all authors
  const [author, setAuthor] = useState({      // Current author being edited/created
    id: "",
    name: "",
    description: "",
    photoUrl: ""
  });
  const [selectedAuthorId, setSelectedAuthorId] = useState(null); // For edit/delete operations
  ...
  const deleteAuthor = (authorId) => {
    axios.delete(apiUrl(routes.AUTHOR, methods.DELETE, authorId))
      .then((response) => {
        toast.success(`Author and ${response.data.deletedBooksCount} associated books deleted`);
        handleCloseDialog();
        handleCloseMenu();
        console.log(response.data);
        getAllAuthors();
