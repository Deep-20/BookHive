...
// Component for handling both creation and updating of book borrowals
// Props include handlers for add/update, modal state management, and borrowal data
const BorrowalForm = ({
                        handleAddBorrowal,
  ...
  // Fetches all members from API, filters based on user role
  // Admins see all members, regular users only see themselves
  const getAllMembers = () => {
  ...
  // Validates borrowal dates to ensure logical order:
  // borrowed date < due date < return date (if exists)
  const validateDates = () => {
    const borrowedDateTime = new Date(borrowal.borrowedDate).getTime();
    const dueDateTime = new Date(borrowal.dueDate).getTime();
    const returnDateTime = borrowal.returnDate ? new Date(borrowal.returnDate).getTime() : null;

    if (dueDateTime < borrowedDateTime) {
      toast.error("Due date cannot be earlier than borrowed date");
      return false;
    }

    if (returnDateTime && returnDateTime < borrowedDateTime) {
      toast.error("Return date cannot be earlier than borrowed date");
      return false;
    }
        ...
        <Container>
          ...
          <Stack spacing={3} paddingY={2}>
            ...
            <Grid container spacing={0} sx={{paddingBottom: "4px"}}>
              ...
              <Grid item xs={12} md={4} paddingRight={1}>
                <TextField fullWidth name="returnDate" label="Return date" type="date" value={borrowal.returnDate}
                           InputLabelProps={{shrink: true}}
                           onChange={(e) => setBorrowal({...borrowal, returnDate: e.target.value})}/>
              </Grid>
            </Grid>
